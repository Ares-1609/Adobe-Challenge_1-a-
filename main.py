...
def identify_headings(pdf_content: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    for page in pages:
        ...
        for line in lines:
            ...
            # 💡 Additional filters to avoid paragraph text:
            if len(line_text) > 100:  # too long, likely a paragraph
                continue
            if re.search(r"\w+\s+\w+\s+\w+.*[.!?]$", line_text):  # ends like a sentence
                continue
            if line_text.count(" ") > 10:  # too many words, likely not a heading
                continue
            if is_likely_body_text(line_text):
                continue
    ...
    # Fallback: use metadata title only if no title found heuristically
    if not document_title and pdf_content["metadata"]["title"]:
